<template>
	<Teleport to="body">
		<div v-if="modal.openComment" class="modal">
			<div class="addForm">
				<v-input
					class="input"
					:inputType="inputType"
				...
				<!-- <v-select
					...
					class="select"
				/> -->
				<v-button class="submitButton" @click="addComment()">Add Comment</v-button>
			</div>
			<div class="close">
				<v-button class="closeButton" @click="modal.openComment = false">x</v-button>
			</div>
		</div>
	</Teleport>
</template>
...
const addComment = () => {
	...
	fetch("http://localhost:9000/board/task/comment", {
		method: "POST",
		body: formData,
		credentials: 'include',
	})
